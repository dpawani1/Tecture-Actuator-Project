...
// Speed is now variable so GUI can change it
int SPEED_US_PER_SEC = 140;
...
// Safe speed limits for GUI control
const int MIN_SPEED_US_PER_SEC = 0;
const int MAX_SPEED_US_PER_SEC = 300;
...
/*
  Always treat incoming number as a MASK (0..511).
*/
...
void handleSpeedCommand(String line) {
  // expects format SPD:123
  String s = line.substring(4);
  int newSpeed = s.toInt();

  if (newSpeed < MIN_SPEED_US_PER_SEC) newSpeed = MIN_SPEED_US_PER_SEC;
  if (newSpeed > MAX_SPEED_US_PER_SEC) newSpeed = MAX_SPEED_US_PER_SEC;

  SPEED_US_PER_SEC = newSpeed;

  Serial.print("NEW_SPEED_US_PER_SEC=");
  Serial.println(SPEED_US_PER_SEC);
}

void handleMaskCommand(String line) {
  long v = line.toInt();
...
void setup() {
  ...
  Serial.println("  MASK 0..511");
  Serial.println("  SPD:<value>  where value is clamped to safe range");
