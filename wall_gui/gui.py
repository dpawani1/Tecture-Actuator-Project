...
class WallGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Entry(conn_frame, textvariable=self.port_var, width=20).grid(row=0, column=1, sticky="w", padx=5)
        ...
        ttk.Button(conn_frame, text="Connect", command=self.connect).grid(row=1, column=0, columnspan=2, pady=(10, 0), sticky="ew")
        ttk.Button(conn_frame, text="Disconnect", command=self.disconnect).grid(row=1, column=2, columnspan=2, pady=(10, 0), sticky="ew")
        ...
        pattern_menu.grid(row=0, column=1, sticky="w", padx=5)

        ttk.Label(pattern_frame, text="Delay (s):").grid(row=1, column=0, sticky="w", pady=(10, 0))
        ttk.Entry(pattern_frame, textvariable=self.delay_var, width=10).grid(row=1, column=1, sticky="w", padx=5, pady=(10, 0))

        ttk.Label(pattern_frame, text="Loops:").grid(row=2, column=0, sticky="w", pady=(10, 0))
        ttk.Entry(pattern_frame, textvariable=self.loops_var, width=10).grid(row=2, column=1, sticky="w", padx=5, pady=(10, 0))
        ...
        # Manual section
        manual_frame = ttk.LabelFrame(main, text="Quick Controls", padding=pad)
        manual_frame.pack(fill="x", pady=(0, 10))

        ttk.Button(manual_frame, text="Home", command=self.home).grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        ttk.Button(manual_frame, text="All On", command=self.all_on).grid(row=0, column=1, sticky="ew", padx=5, pady=5)
        ttk.Button(manual_frame, text="Stop", command=self.stop).grid(row=0, column=2, sticky="ew", padx=5, pady=5)

        ttk.Button(manual_frame, text="Run Demo Mode", command=self.run_demo).grid(
            row=1, column=0, columnspan=3, sticky="ew", padx=5, pady=5
        )

        for i in range(3):
            manual_frame.columnconfigure(i, weight=1)
    ...
    def home(self):
        try:
            ...
            self.status_var.set("Homing / all actuators off.")
        except Exception as e:
    ...
    def all_on(self):
        try:
            ...
            self.status_var.set("All actuators commanded ON.")
        except Exception as e:
    ...
    def stop(self):
        try:
            ...
            self.status_var.set("Stopped. Sent home mask.")
        except Exception as e:
    ...
    def run_pattern(self):
        try:
            ...
            self.controller.play_pattern(pattern, loops=loops, frame_delay=delay)
    ...
    def run_demo(self):
        try:
            self.controller.play_demo()
            self.status_var.set("Running demo mode.")
        except Exception as e:
    ...
    def _poll_status(self):
        try:
            ...
            if status["connected"]:
                self.status_var.set(
                    f"Connected | Mode: {status['mode']} | Busy: {status['busy']}"
                )
            else:
